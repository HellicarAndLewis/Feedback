...
//------------------------------------------------------------------------------------
void vectorField::limit(float maxValue) {
	for (int i = 0; i < fieldSize; i++){
		field[i].limit(maxValue);
	}
}
...
//------------------------------------------------------------------------------------
void vectorField::setValue(float x, float y, float vx, float vy) {
	...
	field[fieldPosY * fieldWidth + fieldPosX].x = vx;
	field[fieldPosY * fieldWidth + fieldPosX].y = vy;
}

//------------------------------------------------------------------------------------
void vectorField::addToField(float x, float y, float vx, float vy) {
	float pctx			= x / (float)externalWidth;
	float pcty			= y / (float)externalHeight;
	int fieldPosX		= (int)(pctx * (float)fieldWidth);
    int fieldPosY		= (int)(pcty * (float)fieldHeight);
